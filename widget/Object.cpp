...
void Object::CalculaNormalsVert()
{
	float auxx=0;
	float auxy=0;
	float auxz=0;
	...
	bool cent=false;
  	for(unsigned int i=0; i<vertices.size(); i++){
		...
		for(unsigned int j=0;j<faces.size();j++){
			k=0;
			cent=false;
			while(k<=faces[j].vertices.size() && cent==false){
				if (((unsigned int)faces[j].vertices[k])==i){
					cares++;
					auxx+=faces[j].normal.x;
					auxy+=faces[j].normal.y;
					auxz+=faces[j].normal.z;
					cent=true;
				}
				k++;
			}
		...
		auxy=auxy/cares;
		auxz=auxz/cares;
		vertices[i].normal.x=auxx;
		vertices[i].normal.y=auxy;
		vertices[i].normal.z=auxz;
	}
}
